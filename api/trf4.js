...
exports.handler = async function(event, context) {
    ...
    if (event.httpMethod === 'GET') {
        return {
            ...
            body: JSON.stringify({
                success: true,
                message: 'API TRF4 - Consulta de precatórios',
                tribunal: 'TRF4 - 4ª Região',
                status: 'online'
            })
        };
    }
    ...
    try {
        ...
        console.log(`📝 TRF4 - Consultando: ${documento}`);

        const API_KEY = 'cDZHYzlZa0JadVREZDJCendQbXY6SkJlTzNjLV9TRENyQk1RdnFKZGRQdw==';
        const endpoint = 'https://api-publica.datajud.cnj.jus.br/api_publica_trf4/_search';
        ...
        console.log(`🌐 TRF4 - Chamando DataJud...`);
        ...
        console.log(`📥 TRF4 - Status DataJud: ${response.status}`);

        if (!response.ok) {
            const errorText = await response.text();
            console.error(`❌ TRF4 - Erro DataJud: ${response.status}`, errorText);
            return {
                statusCode: 200, // Retorna 200 para não quebrar o frontend
                headers,
                body: JSON.stringify({
                    success: true,
                    tribunal: 'TRF4 - 4ª Região',
                    total: 0,
                    processos: [],
                    aviso: `DataJud TRF4 indisponível no momento (${response.status})`
                })
            };
        }

        const data = await response.json();
        console.log(`✅ TRF4 - DataJud respondeu! Hits: ${data.hits?.total?.value || 0}`);
        ...
        return {
            ...
            body: JSON.stringify({
                ...
                total: 0,
                processos: [],
                aviso: 'Erro interno no TRF4, tente novamente mais tarde'
            })
        };
    }
};
